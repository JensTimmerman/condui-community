...
export function getInfoBlockReservedZoneMm(
  ...
  const aspect = INFO_BLOCK_HEIGHT / INFO_BLOCK_TOTAL_WIDTH
...
export function getPdfContentHeightMm(
  orientation: 'landscape' | 'portrait',
  options: { hasInfoBlock: boolean; hasPanelTitle: boolean }
): number {
  ...
  if (options.hasInfoBlock) {
    height -= getInfoBlockReservedZoneMm(orientation).heightMm + INFO_BLOCK_GAP_MM
  }
