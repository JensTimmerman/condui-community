...
export function ExportDialog({ onExport, onCancel }: CommunityExportDialogProps) {
  ...
  const options = useMemo<ExportOptions>(() => ({
    ...
    theme,
  }), [includeEendraad, includeInstallDates, includePanel, includeSitplan, theme])
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="space-y-2">
          <label className="flex cursor-pointer items-center gap-2">
            <input type="checkbox" checked={includeEendraad} onChange={(event) => setIncludeEendraad(event.target.checked)} />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t('export.dialog.eendraad', '1draad (Single-line diagram)')}</span>
          </label>
          <label className={`ml-6 flex items-center gap-2 ${includeEendraad ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
            <input type="checkbox" checked={includeEendraad && includeInstallDates} disabled={!includeEendraad} onChange={(event) => setIncludeInstallDates(event.target.checked)} />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t('export.dialog.installDates', 'Include install dates')}</span>
          </label>
          <label className="flex cursor-pointer items-center gap-2">
            <input type="checkbox" checked={includePanel} onChange={(event) => setIncludePanel(event.target.checked)} />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t('export.dialog.panel', 'Panel (Cabinet view)')}</span>
          </label>
          <label className="flex cursor-pointer items-center gap-2">
            <input type="checkbox" checked={includeSitplan} onChange={(event) => setIncludeSitplan(event.target.checked)} />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t('export.dialog.sitplan', 'Sitplan (Floor plan)')}</span>
          </label>
        </div>
      </div>

      <div className="border-t border-gray-200 pt-3 dark:border-gray-700">
        <h4 className="mb-2 text-sm font-medium text-gray-900 dark:text-gray-100">{t('export.dialog.theme', 'Export theme')}</h4>
        <CustomDropdown
          value={theme}
      ...
      <div className="border-t border-gray-200 pt-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">
        <div>{t('export.dialog.totalPages', 'Total: {{count}} pages', { count: pageCounts.total })}</div>
      </div>

      <div className="flex gap-3 pt-2">
        <button onClick={onCancel} className="flex-1 rounded-md border border-gray-300 px-4 py-2 font-medium text-gray-700 dark:border-gray-600 dark:text-gray-300">
